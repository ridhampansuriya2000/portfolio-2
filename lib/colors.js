...
//
// `colorAt(index)` cycles through the first CYCLE_LENGTH entries only, so
// appending a new named color (e.g. for a specific brand) never reshuffles
// the colors every other section already got via automatic cycling.
...
export const palette = [
  ...
  // Named-only colors below: never participate in colorAt()'s automatic
  // cycle, only picked deliberately via colorByName (e.g. to match a
  // product's own brand color).
  {
    name: "blue",
    text: "text-blue-700 dark:text-blue-300",
    icon: "text-white dark:text-blue-300",
    iconBg: "bg-gradient-to-br from-blue-400 to-blue-600 shadow-md shadow-blue-500/30 dark:bg-blue-500/15 dark:from-transparent dark:to-transparent dark:shadow-none",
    card: "bg-blue-50 border border-blue-200/80 shadow-sm shadow-blue-200/50 backdrop-blur-sm dark:bg-white/[0.035] dark:border-white/10 dark:shadow-none dark:backdrop-blur-lg",
    border: "hover:border-blue-400 dark:hover:border-blue-400/50",
    tag: "border-blue-300 bg-white/70 text-blue-700 dark:border-blue-400/30 dark:bg-blue-500/10 dark:text-blue-200",
    glow: "hover:shadow-lg hover:shadow-blue-500/20",
  },
];

export function colorAt(index) {
  return palette[index % CYCLE_LENGTH];
}

export function colorByName(name) {
  return palette.find((c) => c.name === name) ?? colorAt(0);
}
